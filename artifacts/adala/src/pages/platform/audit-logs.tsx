...
export default function AuditLogsPage() {
  ...
  const { data: logs, isLoading, refetch } = useQuery<{ rows: any[]; total: number; pages: number }>({
    ...
    queryFn: () => authFetch(`${BASE}/api/audit-logs?${params}`).then(r => { if (!r.ok) throw new Error("خطأ في الخادم"); return r.json(); }),
  });

  const { data: stats } = useQuery<any>({
    queryKey: ["audit-logs-stats"],
    queryFn: () => authFetch(`${BASE}/api/audit-logs/stats`).then(r => { if (!r.ok) throw new Error("خطأ في الخادم"); return r.json(); }),
    ...
    <div className="flex flex-col h-full gap-0">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-foreground">سجل المراقبة والأمان</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <div className="space-y-1">
              {stats.suspicious.map((s: any, i: number) => (
                <p key={i} className="text-xs text-white/70">
                  المستخدم <span className="text-foreground font-medium">{s.user_full_name || s.user_id || "مجهول"}</span> قام
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Input type="date" value={from} onChange={e => { setFrom(e.target.value); filterChanged(); }}
            className="h-8 w-36 text-xs bg-muted/50 border-border text-white" />
                  ...
                  return (
                    <tr key={row.id} className="border-b border-border/50 hover:bg-accent/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-white/80 max-w-[120px] truncate">
                        {row.user_full_name || row.user_id || <span className="text-muted-foreground/60 italic">نظام</span>}
                      </td>
        ...
        {((stats?.byAction?.length ?? 0) > 0) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              ...
              { title: "الموارد (آخر 7 أيام)", data: stats?.byResource, key: "resource", labelFn: (v: string) => RESOURCE_LABELS[v] ?? v, color: "#2563EB" },
            ].map(chart => (
