      ...
      <div className="p-6 space-y-5 max-w-5xl mx-auto">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-bold text-white">الحسابات البنكية</h1><p className="text-xs text-muted-foreground">إدارة الحسابات والأرصدة</p></div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <Card className="bg-sidebar border-sidebar-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">إجمالي الأرصدة</p><p className="text-xl font-bold text-[#C9A84C]">{fmt(totalBalance)}</p></CardContent></Card>
          <Card className="bg-sidebar border-sidebar-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">عدد الحسابات</p><p className="text-2xl font-bold text-white">{rows.length}</p></CardContent></Card>
          <Card className="bg-sidebar border-sidebar-border"><CardContent className="p-4"><p className="text-xs text-muted-foreground mb-1">الحساب الافتراضي</p><p className="text-sm font-medium text-white truncate">{rows.find(r => r.isDefault)?.bankName ?? "غير محدد"}</p></CardContent></Card>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoading ? (
            ...
            <div className="col-span-full flex flex-col items-center py-14 text-muted-foreground"><Landmark className="h-10 w-10 mb-2 opacity-20" /><p className="text-sm">لا توجد حسابات</p><Button size="sm" variant="link" className="text-[#C9A84C] mt-1" onClick={openCreate}>إضافة أول حساب</Button></div>
          ) : rows.map(r => (
            <Card key={r.id} className={`bg-sidebar border-sidebar-border hover:border-[#C9A84C]/30 transition-colors ${r.isDefault ? "border-[#C9A84C]/40" : ""}`}>
              <CardContent className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5">
                      <p className="text-sm font-bold text-white truncate">{r.bankName}</p>
                      {r.isDefault && <Star className="h-3 w-3 text-[#C9A84C] fill-[#C9A84C] shrink-0" />}
                    </div>
                    <p className="text-xs text-muted-foreground truncate">{r.accountName}</p>
                  </div>
                  <Badge variant="outline" className="border-[#C9A84C]/30 text-[#C9A84C] text-[10px] shrink-0">{r.currency}</Badge>
                </div>
                <div>
                ...
                <div className="pt-1 border-t border-sidebar-border flex items-center justify-between">
                  <div><p className="text-xs text-muted-foreground">الرصيد</p><p className="text-lg font-bold text-[#C9A84C]">{fmt(r.currentBalance)}</p></div>
            ...
            <div className="space-y-3 py-2">
              <div><Label className="text-xs text-muted-foreground">اسم البنك *</Label>
                <select value={form.bankName ?? ""} onChange={e => set("bankName", e.target.value)}
                  className="w-full bg-background/50 border border-sidebar-border rounded-md px-3 py-2 text-sm text-white mt-1">
                  {BANKS.map(b => <option key={b} value={b}>{b}</option>)}
                </select></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label className="text-xs text-muted-foreground">اسم الحساب *</Label><Input value={form.accountName ?? ""} onChange={e => set("accountName", e.target.value)} className="bg-background/50 border-sidebar-border mt-1 text-sm" /></div>
                <div><Label className="text-xs text-muted-foreground">رقم الحساب *</Label><Input value={form.accountNumber ?? ""} onChange={e => set("accountNumber", e.target.value)} className="bg-background/50 border-sidebar-border mt-1 text-sm" dir="ltr" /></div>
              </div>
              <div><Label className="text-xs text-muted-foreground">IBAN (اختياري)</Label><Input value={form.iban ?? ""} onChange={e => set("iban", e.target.value)} className="bg-background/50 border-sidebar-border mt-1 text-sm" dir="ltr" placeholder="SA..." /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label className="text-xs text-muted-foreground">الرصيد الحالي (ر.س)</Label><Input type="number" value={form.currentBalance ?? ""} onChange={e => set("currentBalance", e.target.value)} className="bg-background/50 border-sidebar-border mt-1 text-sm" /></div>
                <div className="flex items-center justify-between rounded-lg border border-sidebar-border p-3 mt-1">
