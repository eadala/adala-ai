    ...
    <div className="min-h-screen bg-muted/30 p-6 rtl" dir="rtl">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => { qc.invalidateQueries({ queryKey: ["fe-summary"] }); }}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs bg-card border border-border text-muted-foreground hover:bg-muted/30 transition">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            ...
            <div className="bg-card border border-border rounded-2xl p-5">
              ...
              <div className="space-y-2">
                {recent.length === 0 && <div className="text-xs text-muted-foreground text-center py-4">لا توجد معاملات بعد</div>}
                {recent.map((tx: any) => (
                  <div key={tx.id} className="flex items-center gap-2 py-1.5 border-b border-gray-50">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded-full ${STATUS_STYLE[tx.status] ?? "bg-gray-100 text-muted-foreground"}`}>{tx.status}</span>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  <tbody>
                    {(officesQ.data?.offices ?? []).map((o: any) => (
                      <tr key={o.office_id} className="border-b border-gray-50 hover:bg-muted/30">
                        ...
                        <td className="py-2 text-muted-foreground text-xs">{new Date(o.last_activity).toLocaleDateString("ar-SA")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {tab === "transactions" && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          {txQ.isLoading && <div className="p-8 text-center text-muted-foreground text-sm">جارٍ التحميل…</div>}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {(txQ.data?.transactions ?? []).map((tx: any) => (
                  <tr key={tx.id} className="border-b border-gray-50 hover:bg-muted/30">
                    ...
                    <td className="py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${STATUS_STYLE[tx.status] ?? "bg-gray-100 text-muted-foreground"}`}>{tx.status}</span>
                    </td>
                    <td className="py-3 text-xs text-muted-foreground">{new Date(tx.created_at).toLocaleDateString("ar-SA")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {(txQ.data?.transactions ?? []).length === 0 && !txQ.isLoading && (
              <div className="p-8 text-center text-muted-foreground text-sm">لا توجد معاملات</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === "ledger" && (
        <div className="bg-card border border-border rounded-2xl overflow-hidden">
          {ledgerQ.isLoading && <div className="p-8 text-center text-muted-foreground text-sm">جارٍ التحميل…</div>}
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-muted/30 border-b border-border/50">
                <tr className="text-xs text-muted-foreground">
            ...
            {(ledgerQ.data?.entries ?? []).length === 0 && !ledgerQ.isLoading && (
              <div className="p-8 text-center text-muted-foreground text-sm">دفتر الأستاذ فارغ — أدخل معاملة اختبارية لترى النتيجة</div>
            )}
          </div>
        </div>
      )}
