...
type InvoicePayment = {
  id: string; invoiceId: string; officeId: string;
  amount: number; method: string; notes?: string;
  recordedBy?: string; paidAt: string; createdAt: string;
};

const STATUS_MAP: Record<string, { label: string; cls: string; icon: any }> = {
  draft:          { label: "مسودة",        cls: "bg-muted/30 text-muted-foreground border-gray-500/20",           icon: FileText },
  ...
  partially_paid: { label: "مدفوعة جزئياً", cls: "bg-teal-500/10 text-teal-400 border-teal-500/20",              icon: Banknote },
...
/** القاعدة: القيم بالريال SAR — ممنوع ÷100 هنا (التحويل لـ Stripe يكون في الـ backend فقط) */
  ...
  const data = useMemo(() => {
    ...
    invoices.forEach(inv => {
      ...
      if (inv.status === "paid" || inv.status === "partially_paid")
        map[key].paid += Number(inv.amountPaid ?? (inv.status === "paid" ? inv.total : 0));
    });
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" dir="rtl">
        ...
        {step === "form" && (<div className="space-y-5 pt-2">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2 flex items-center justify-between rounded-xl border border-border/40 bg-muted/20 px-4 py-3">
              <div>
                <p className="text-sm font-medium">ضريبة القيمة المضافة (VAT)</p>
                <p className="text-xs text-muted-foreground">تُطبَّق على إجمالي الفاتورة</p>
              </div>
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => setTaxEnabled(v => !v)}
                  className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none ${taxEnabled ? "bg-primary" : "bg-muted"}`}
                >
                  <span className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-lg transition-transform ${taxEnabled ? "translate-x-4" : "translate-x-0"}`} />
                </button>
                {taxEnabled && (
                  <Select value={String(vatRate)} onValueChange={v => setVatRate(Number(v))}>
                    <SelectTrigger className="w-28 h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="15">15% — قياسية</SelectItem>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-12 gap-2 text-xs text-muted-foreground px-1">
              ...
              <span className="col-span-3 text-center">السعر (ر.س)</span>
            ...
            {items.map((item, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center">
                ...
                <Input className="col-span-3 text-sm text-center" type="number" min={0} step="0.01" placeholder="0.00"
...
function InvoiceSheet({
  ...
  const METHOD_LABELS: Record<string, string> = {
    bank: "تحويل بنكي", cash: "نقداً", card: "بطاقة ائتمانية",
    cheque: "شيك", transfer: "حوالة", stripe: "Stripe", other: "أخرى",
  };
  ...
  const recordPayment = useMutation({
    mutationFn: () => fetch(`${BASE}/api/invoices/${invoice!.id}/payments`, {
      ...
      const d = await r.json();
      if (!r.ok) throw new Error(d?.error?.message ?? "خطأ في الخادم");
      return d;
    }),
    onSuccess: (d) => {
      toast.success(`تم تسجيل الدفعة ✅ — المتبقي: ${fmt(d.remaining)} ر.س`);
      setPayDialogOpen(false);
      setPayAmount(""); setPayNotes(""); setPayMethod("bank");
      refetchPayments();
      onRefresh();
    },
            ...
            <div className="space-y-3 pt-1">
              <div className="flex items-center justify-between">
                <p className="text-sm font-semibold flex items-center gap-1.5">
                  <Banknote className="h-4 w-4 text-primary" />سجل الدفعات
                </p>
                {invoice.status !== "paid" && (
                  <Button size="sm" variant="outline"
                    className="gap-1.5 text-xs text-primary border-primary/30 h-7 px-2.5"
                    onClick={() => setPayDialogOpen(v => !v)}>
              ...
              {paymentsData && (
                <div className="space-y-1.5">
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>المدفوع: <span className="font-mono text-foreground">{fmt(paymentsData.amountPaid)} ر.س</span></span>
                    <span>المتبقي: <span className={`font-mono ${paymentsData.remaining > 0 ? "text-amber-400" : "text-green-400"}`}>{fmt(paymentsData.remaining)} ر.س</span></span>
                  </div>
                  <div className="h-2 bg-muted/50 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full transition-all ${paymentsData.remaining <= 0 ? "bg-green-500" : "bg-primary"}`}
                      style={{ width: `${Math.min(100, (paymentsData.amountPaid / paymentsData.total) * 100)}%` }}
                    />
                  </div>
                </div>
              )}
              ...
              {/* نموذج تسجيل دفعة */}
              {payDialogOpen && invoice.status !== "paid" && (
                <div className="border border-primary/20 bg-primary/5 rounded-xl p-4 space-y-3">
                  <p className="text-xs font-semibold text-primary">تسجيل دفعة جديدة</p>
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <Label className="text-xs">المبلغ (ر.س)</Label>
                      <Input
                        type="number" step="0.01" min="0.01"
                        placeholder={paymentsData ? fmt(paymentsData.remaining) : "0.00"}
                        value={payAmount}
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="flex-1 gap-1.5 text-xs bg-primary hover:bg-primary/90"
                      ...
                      disabled={!payAmount || isNaN(parseFloat(payAmount)) || parseFloat(payAmount) <= 0 || recordPayment.isPending}>
                      {recordPayment.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}
                      تأكيد الدفعة
                    </Button>
                    <Button size="sm" variant="ghost" className="text-xs"
                      onClick={() => { setPayDialogOpen(false); setPayAmount(""); setPayNotes(""); }}>
                      إلغاء
                    </Button>
                  </div>
                </div>
              ...
              {paymentsData?.payments && paymentsData.payments.length > 0 ? (
                <div className="space-y-1.5 max-h-48 overflow-y-auto">
                  {paymentsData.payments.map((pay) => (
                    <div key={pay.id} className="flex items-center justify-between bg-muted/30 rounded-lg px-3 py-2">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-mono font-semibold text-green-400">{fmt(pay.amount)} ر.س</span>
                          <Badge variant="outline" className="text-[10px] h-4 px-1.5">{METHOD_LABELS[pay.method] ?? pay.method}</Badge>
                        </div>
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6 text-red-400/60 hover:text-red-400 shrink-0"
                        onClick={() => { if (window.confirm("حذف هذه الدفعة؟")) deletePayment.mutate(pay.id); }}>
                ...
                paymentsData && (
                  <p className="text-xs text-muted-foreground text-center py-2">لا توجد دفعات مسجّلة</p>
                )
              )}
            </div>
          )}
