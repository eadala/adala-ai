...
assert.match(mig, /reason_code=INCOMPATIBLE_FK/);
assert.match(mig, /marketplace_orders\.notes TEXT missing|marketplace_orders','notes','text/);
...
assert.match(pre, /FK_VALIDATION_PENDING/);
assert.match(pre, /pending_fk_validation/);
assert.match(pre, /INCOMPATIBLE_(?:TYPE|PK|UNIQUE|FK)/);
assert.match(pre, /DUPLICATE_UNIQUE_KEY/);
assert.match(pre, /marketplace_orders','notes','text/);
