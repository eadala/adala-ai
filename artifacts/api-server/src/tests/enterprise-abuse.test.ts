...
const FRONTEND = resolve(__dirname, "../../../adala/src");
...
function testFrontendNavPermissionGates() {
  const layout = readFileSync(resolve(FRONTEND, "components/layout.tsx"), "utf8");
...
function testStrictEnforcementDocumented() {
  const env = readFileSync(resolve(__dirname, "../../../../.env.example"), "utf8");
  assert.match(env, /AUTHORIZATION_ENFORCEMENT/);
  console.log("  ✅ AUTHORIZATION_ENFORCEMENT documented in .env.example");
}
...
function main() {
  ...
  testStrictEnforcementDocumented();
