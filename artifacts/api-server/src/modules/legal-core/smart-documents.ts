...
async function analyzeDocument(fileData: string, fileType: string, fileName: string): Promise<Record<string, any> | null> {
  if (!GEMINI_KEY) return null;
  const isVisual = fileType.startsWith("image/") || fileType === "application/pdf";
  const base64   = fileData.includes(",") ? fileData.split(",")[1] : fileData;
  const prompt   = `أنت محلل قانوني خبير. حلل هذا المستند القانوني.
أجب بـ JSON صالح فقط بدون أي نص خارجه:
{
  "summary": "ملخص المستند في 3-5 جمل",
  "document_type": "نوع المستند: عقد أو صك أو حكم أو مذكرة أو وثيقة أو إشعار أو شهادة أو أخرى",
  "parties": ["الطرف الأول", "الطرف الثاني"],
  "dates": ["التواريخ المذكورة"],
  "amounts": ["المبالغ المالية مع العملة"],
  "deed_numbers": ["أرقام الصكوك أو المراجع"],
  "keywords": ["5-8 كلمات مفتاحية قانونية"],
  "risk_notes": "المخاطر أو الملاحظات القانونية المهمة أو null"
}
اسم الملف: ${fileName}`;
  try {
    const parts: any[] = isVisual && base64
      ? [{ inline_data: { mime_type: fileType, data: base64 } }, { text: prompt }]
      : [{ text: `${prompt}\n\n(تحليل بناءً على اسم الملف — المحتوى غير متاح للقراءة)` }];
...
router.post("/smart-documents/cloud-connections", requireAuthWithTenant, async (req, res) => {
  try {
    ...
    const result = await db.execute(sql`
      INSERT INTO cloud_storage_connections (office_id, provider, display_name, account_email, account_name)
      VALUES (${tenantId}, ${provider}, ${displayName ?? null}, ${accountEmail ?? null}, ${accountName ?? null})
...
router.delete("/smart-documents/cloud-connections/:id", requireAuthWithTenant, async (req, res) => {
  try {
    ...
    await db.execute(sql`
      UPDATE cloud_storage_connections SET is_active = FALSE, updated_at = NOW()
...
router.post("/smart-documents", requireAuthWithTenant, async (req, res) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const { fileName, fileType, fileData, fileSize, caseId, clientId, contractId, uploadedBy, notes, autoAnalyze } = req.body;
    ...
    if (autoAnalyze && fileData && GEMINI_KEY) {
      analysis = await analyzeDocument(fileData, fileType ?? "application/octet-stream", fileName);
    }

    const insertResult = await db.execute(sql`
      INSERT INTO smart_documents (
        office_id, case_id, client_id, contract_id,
        file_name, file_type, file_size, cloud_provider, uploaded_by, notes,
        ai_analyzed, ai_summary, ai_parties, ai_dates, ai_amounts,
        ai_document_type, ai_keywords, ai_deed_numbers, ai_risk_notes
      ) VALUES (
        ${tenantId},
        ${caseId ?? null}, ${clientId ?? null}, ${contractId ?? null},
        ${fileName}, ${fileType ?? "application/octet-stream"}, ${fileSize ?? 0},
      ...
      RETURNING id, file_name, file_type, file_size, ai_analyzed,
                ...
                ai_document_type, ai_keywords, ai_deed_numbers, ai_risk_notes, created_at
    `);
    const created = rows(insertResult)[0];
    if (!created) throw new Error("فشل إنشاء المستند");
    auditLog({ ...auditMeta(req), action: "upload", resource: "smart_document", resourceId: String(created.id), details: `ملف: ${fileName}` }).catch(() => {});
    res.status(201).json({
      ...created,
...
router.post("/smart-documents/:id/analyze", requireAuthWithTenant, async (req, res) => {
  try {
    ...
    if (!GEMINI_KEY) return res.status(400).json({ error: "خدمة التحليل الذكي غير متاحة" });
    const docRes = await db.execute(sql`
      SELECT id, file_name, file_type FROM smart_documents
      WHERE id = ${id}::uuid AND office_id = ${tenantId}
    `);
    const doc = rows(docRes)[0];
    if (!doc) return res.status(404).json({ error: "المستند غير موجود" });
    const analysis = await analyzeDocument(fileData ?? "", fileType ?? doc.file_type, nameOverride ?? doc.file_name);
    if (!analysis) return res.status(500).json({ error: "فشل تحليل المستند" });
    await db.execute(sql`
...
router.delete("/smart-documents/:id", requireAuthWithTenant, async (req, res) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const id       = String((req.params as Record<string, string>).id);
    await db.execute(sql`DELETE FROM smart_documents WHERE id = ${id}::uuid AND office_id = ${tenantId}`);
