...
/* ── Stats overview ── */
router.get("/audit-logs/stats", requireAuthWithTenant, async (req, res) => {
  try {
    const tenantId = (req as any).tenantId as string;
    const today = await sqlOne(sql`
      SELECT COUNT(*)::int AS total FROM audit_logs WHERE office_id = ${tenantId} AND created_at >= CURRENT_DATE
    `);
    const users = await sqlOne(sql`
      SELECT COUNT(DISTINCT user_id)::int AS total FROM audit_logs WHERE office_id = ${tenantId} AND created_at >= CURRENT_DATE
    `);
    const deletions = await sqlOne(sql`
      SELECT COUNT(*)::int AS total FROM audit_logs WHERE office_id = ${tenantId} AND action = 'delete' AND created_at >= CURRENT_DATE
    `);
    const byAction = await sqlAll(sql`
      SELECT action, COUNT(*)::int AS count FROM audit_logs
      WHERE office_id = ${tenantId} AND created_at >= NOW() - INTERVAL '7 days'
    ...
    const hourly = await sqlAll(sql`
      SELECT
        ...
        COUNT(*)::int AS count
      FROM audit_logs WHERE office_id = ${tenantId} AND created_at >= NOW() - INTERVAL '24 hours'
    ...
    const suspicious = await sqlAll(sql`
      ...
      FROM audit_logs
      WHERE office_id = ${tenantId} AND action = 'delete' AND created_at >= NOW() - INTERVAL '1 hour'
...
/* ── Paginated log list ── */
router.get("/audit-logs", requireAuthWithTenant, async (req, res) => {
  try {
    const tenantId = (req as any).tenantId as string;
