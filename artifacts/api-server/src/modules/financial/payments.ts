...
router.get("/payments/transactions", requireAuthWithTenant, requirePermission("payments:view"), async (_req, res) => {
  try {
...
/* backwards compat */
router.get("/payments/stats", requireAuthWithTenant, requirePermission("payments:view"), async (_req, res) => {
  try {
    const totals = await one(sql`
      SELECT
        COUNT(*)::int                                 AS total_transactions,
