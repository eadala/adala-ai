...
# 45) Migration 045 — Support AI schema authority (Stage 7)
#    Ops:
#      a) run preflight-migration-045.sql
#      b) if BLOCK_AND_MANUAL_REVIEW → stop
#      c) if SAFE_AUTO_REPAIR or ALREADY_CORRECT → apply 045
#      d) re-run preflight → expect ALREADY_CORRECT
#      e) bash scripts/db/verify-schema.sh
#      f) deploy API
#    Owns: support_ai_analysis (+ exact UNIQUE(ticket_id) for
#    ON CONFLICT (ticket_id) DO UPDATE) + support_knowledge_base (PK only).
#    Does NOT invent KB business UNIQUE/FK. Does NOT ALTER ai_score /
#    Enterprise support objects / 039–044.
#    Runtime CREATE removed from ensureSupportAITables; readiness + DML preserved
#    (KB seed duplicate-seed follow-up — no invented UNIQUE).
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-045.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f artifacts/api-server/migrations/045_support_ai_schema_authority.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-045.sql
...
# 47) Migration 047 — Calendar schema authority (Stage 8)
#    Ops:
#      a) run preflight-migration-047.sql
#      b) if BLOCK_AND_MANUAL_REVIEW → stop
#      c) if SAFE_AUTO_REPAIR or ALREADY_CORRECT → apply 047
#      d) re-run preflight → expect ALREADY_CORRECT
#      e) bash scripts/db/verify-schema.sh
#      f) deploy API
#    Owns: events (TEXT PK) + event_reminders (TEXT PK + FK event_id →
#    events(id) ON DELETE CASCADE) + idx_events_case_id (case_id ASC) +
#    idx_events_office_start (office_id, start_at ASC). Same index shapes as
#    020 (020 not rewritten). No invented UNIQUE / case_id FK.
#    office_id TEXT NOT NULL DEFAULT 'default'. Extra live columns left alone.
#    Runtime CREATE removed from calendar.ensureTables; readiness + DML preserved.
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-047.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f artifacts/api-server/migrations/047_calendar_schema_authority.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-047.sql

# 48) Migration 048 — HR Internal schema authority (Stage 8)
#    Ops:
#      a) run preflight-migration-048.sql
#      b) if BLOCK_AND_MANUAL_REVIEW → stop
#      c) if SAFE_AUTO_REPAIR or ALREADY_CORRECT → apply 048
#      d) re-run preflight → expect ALREADY_CORRECT
#      e) bash scripts/db/verify-schema.sh
#      f) deploy API
#    Owns: hr_announcements, employee_requests, leave_balances (+ exact
#    UNIQUE(employee_id, leave_type, year) for ON CONFLICT DO NOTHING).
#    Preserves Runtime defaults/nullability; no invented FK/UNIQUE/index.
#    Runtime CREATE removed from hrInternal.ensureTables; readiness + DML preserved.
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-048.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f artifacts/api-server/migrations/048_hr_internal_schema_authority.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-048.sql
...
# 50) Migration 050 — HR Enterprise schema authority (Stage 8)
#    Ops:
#      a) run preflight-migration-050.sql
#      b) if BLOCK_AND_MANUAL_REVIEW → stop
#      c) if SAFE_AUTO_REPAIR or ALREADY_CORRECT → apply 050
#      d) re-run preflight → expect ALREADY_CORRECT
#      e) bash scripts/db/verify-schema.sh
#      f) deploy API
#    Owns: hr_roles, hr_memberships, hr_workflows, hr_audit_logs
#    (+ UNIQUE(office_id, name) / UNIQUE(office_id, user_id)
#     + idx_hrwf_office / idx_hral_office DESC).
#    Runtime CREATE/INDEX removed from ensureHREnterpriseTables;
#    readiness + seed/ON CONFLICT + tenant DML preserved.
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-050.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f artifacts/api-server/migrations/050_hr_enterprise_schema_authority.sql
psql "$DATABASE_URL" -v ON_ERROR_STOP=1 \
  -f scripts/db/preflight-migration-050.sql
...
| `054_platform_runtime_schema_authority.sql` | Stage 9. Owns former Runtime CREATE/INDEX IIFEs from `control-tower.ts` / `launchGate.ts` / `governanceKernel.ts` / `certification.ts` / `admin.ts` / `engineering.ts` / `production-os.ts` / `productionLaunch.ts` / `saas-os.ts` (13 tables + `idx_ct_sec_events_*` / `idx_gov_log_created` / `idx_sys_audit_*` + UNIQUE `certificate_id` / `ip_address`). Merges `ct_security_events` launchGate + control-tower `message`/`metadata` columns. BLOCK on incompatible type/UNIQUE/INDEX (no DROP). Runtime CREATE/INDEX removed; readiness + DML/SA preserved. Preflight: `scripts/db/preflight-migration-054.sql`. |
| `055_platform_runtime_b2_schema_authority.sql` | Stage 9. Owns former Runtime CREATE/INDEX IIFEs from `developer.ts` / `tenantDebug.ts` / `infrastructure.ts` / `orgStructure.ts` / `managedIntegrations.ts` / `demo-sync.ts` / `platformCommand.ts` / `themeBuilder.ts` / `tenantVersioning.ts` (15 tables + `idx_tenant_audit_*` / `idx_ois_office` / `idx_ir_status` / `idx_tb_*` / `idx_tbh_*` / UNIQUE `idx_taa_tenant_period` + UNIQUE probes for `super_admin_user_id` / `email` / `office_id` / `(office_id, integration_key)` / `user_id`). Adds proven `organization_units.parent_id` FK ON DELETE SET NULL. BLOCK on incompatible type/UNIQUE/INDEX/FK (no DROP). Seed DML for `platform_integrations` and `compressAuditLogs` DELETE preserved in modules. Preflight: `scripts/db/preflight-migration-055.sql`. |
| `056_rls_runtime_schema_authority.sql` | Stage 9. Owns former Runtime RLS DDL from `rls-migration.ts` / `dataVault.ts`: `security_events` (+ indexes) / `rls_enablement_log`; ENABLE+FORCE RLS + `zta_tenant_isolation_*` policies (Zero Trust); ENABLE RLS + `vault_tenant_isolation` policies (Data Vault, proven fallback USING when `adala_tenant_ok` absent). Skips missing tables / tables without `office_id`. BLOCK on incompatible policy/index (no DROP POLICY/INDEX). `office_erp_ledger` RLS remains **013**. Runtime ENABLE/FORCE/CREATE/DROP POLICY removed; readiness + SA/tenant routes + DML preserved. Preflight: `scripts/db/preflight-migration-056.sql`. |
| `057_monitoring_isolation_rls_schema_authority.sql` | Stage 9. Owns former Runtime RLS DDL from `isolation.ts` POST `/isolation/enable-rls`: ENABLE RLS (no FORCE) + `rls_${table}` policies with proven USING (`col::text = current_setting(...) OR coalesce(...) = '' OR bypass_rls = 'on'`). Skips tables with Migration **056** `zta_tenant_isolation_*` / `vault_tenant_isolation`. Discovers public tables with `office_id` or `tenant_id` (prefers `office_id`). BLOCK on incompatible `rls_*` qual (no DROP POLICY). Runtime ENABLE/CREATE/DROP POLICY removed; readiness + Super Admin routes preserved. Preflight: `scripts/db/preflight-migration-057.sql`. |
...
Calendar Runtime (Stage 8 / **047**) P0-gated for calendar/appointments routes: `events`, `event_reminders` (essential columns include `events.office_id` / `events.start_at` / `events.event_type` / `event_reminders.event_id`). Exact indexes `idx_events_case_id` `(case_id)` ASC and `idx_events_office_start` `(office_id, start_at)` ASC. FK `event_reminders.event_id → events(id) ON DELETE CASCADE`. Runtime CREATE removed from `calendar.ensureTables`; readiness + DML preserved.
