...
/* ═════════════════════════════════════════════════════════
   9. Tenant Security Foundation
═════════════════════════════════════════════════════════ */
head("9/11 Tenant Security Foundation");
...
const p0Routes = [
  ...
  ["copilot /chat", "src/modules/ai/copilot.ts", [/router\.post\("\/chat", requireAuthWithTenant/, /requirePermission\("ai:access"\)/]],
  ["AI gateway /ai/query", "src/modules/ai/aiGateway.ts", [/router\.post\("\/ai\/query", requireAuthWithTenant/, /getRequiredTenantId/, /requirePermission\("ai:access"\)/]],
...
const tenantCtxSrc = readSrc(BACKEND, "src/core/tenantContext.ts") ?? "";
if (tenantCtxSrc.includes("tenantRequiredResponse") || reqAuthTenantSrc.includes("TNT_403")) {
  pass("استجابة TNT_403 موحّدة لمسارات tenant-required");
} else {
  warn("TNT_403 response helper غير مؤكد");
  tenantSecWarnings++;
}
...
═════════════════════════════════════════════════════════ */
head("10/11 Authorization Foundation");
let authzIssues = 0;
let authzWarnings = 0;
...
const appAuthzSrc = readSrc(BACKEND, "src/app.ts") ?? "";
if (appAuthzSrc.includes("enforceRoutePolicy")) {
  pass("enforceRoutePolicy مفعّل في app.ts");
} else {
  warn("enforceRoutePolicy غير مثبت في app.ts");
  authzWarnings++;
}
...
/* ═════════════════════════════════════════════════════════
   11. AI Gateway RBAC (PR-AI-002)
═════════════════════════════════════════════════════════ */
head("11/11 AI Gateway RBAC — requirePermission(ai:access)");
let aiAuthzIssues = 0;
...
if (criticalFails === 0 && totalIssues === 0) {
  ...
  console.log(`${C.GREEN}   جميع طبقات المنصة الـ 11 محكومة ومفحوصة${C.RESET}`);
