-- ═══════════════════════════════════════════════════════════════════════════
-- Preflight Migration 045 — READ-ONLY checks for Support AI schema
--
-- This script reads catalogs/data and emits notices only. It does not
-- CREATE / ALTER / DROP durable objects.
--
-- Owned tables (2):
--   support_ai_analysis (+ UNIQUE(ticket_id)),
--   support_knowledge_base (PK only — no invented business UNIQUE)
--
-- ticket_id is a TEXT business key: never UUID-validated.
-- KB duplicate seed rows are reported as notice-only (never preserved).
--
...
--   SUPPORT_AI_SCHEMA_READY means ALREADY_CORRECT.
--
-- On BLOCK: RAISE EXCEPTION so ON_ERROR_STOP scripts fail closed.
-- ═══════════════════════════════════════════════════════════════════════════

\echo '▶ 045 preflight: object presence'
SELECT
  to_regclass('public.support_ai_analysis') IS NOT NULL AS support_ai_analysis_present,
  to_regclass('public.support_knowledge_base') IS NOT NULL AS support_knowledge_base_present;

\echo '▶ 045 preflight: full contract and decision'
DO $preflight$
DECLARE
  owned_tables CONSTANT TEXT[] := ARRAY[
    'support_ai_analysis','support_knowledge_base'
  ]::TEXT[];
...
BEGIN
  ...
  -- ticket_id is intentionally excluded from UUID-shape checks.
  FOR column_spec IN
    SELECT * FROM (VALUES
      ('support_ai_analysis','id','uuid',TRUE,'gen_uuid',NULL),
      ('support_ai_analysis','ticket_id','text',TRUE,NULL,NULL),
      ('support_ai_analysis','ai_type','text',FALSE,NULL,NULL),
      ('support_ai_analysis','ai_priority','text',FALSE,NULL,NULL),
      ('support_ai_analysis','ai_root_cause','text',FALSE,NULL,NULL),
      ('support_ai_analysis','ai_confidence','numeric',FALSE,'literal','0'),
      ('support_ai_analysis','ai_suggestions','jsonb',FALSE,'jsonb_arr',NULL),
      ('support_ai_analysis','ai_summary','text',FALSE,NULL,NULL),
      ('support_ai_analysis','ai_auto_replied','bool',FALSE,'literal','false'),
      ('support_ai_analysis','ai_escalated','bool',FALSE,'literal','false'),
      ('support_ai_analysis','soc_alerted','bool',FALSE,'literal','false'),
      ('support_ai_analysis','knowledge_hits','jsonb',FALSE,'jsonb_arr',NULL),
      ('support_ai_analysis','model_used','text',FALSE,NULL,NULL),
      ('support_ai_analysis','created_at','timestamptz',FALSE,'now',NULL),
      ('support_ai_analysis','updated_at','timestamptz',FALSE,'now',NULL),
      ('support_knowledge_base','id','uuid',TRUE,'gen_uuid',NULL),
      ('support_knowledge_base','category','text',TRUE,NULL,NULL),
      ('support_knowledge_base','issue','text',TRUE,NULL,NULL),
      ('support_knowledge_base','fix','text',TRUE,NULL,NULL),
      ('support_knowledge_base','tags','_text',FALSE,'empty_arr',NULL),
      ('support_knowledge_base','hits','int4',FALSE,'literal','0'),
      ('support_knowledge_base','created_at','timestamptz',FALSE,'now',NULL)
    ) AS expected_column(
  ...
  -- PK (id) sole-column probe for both tables
  FOREACH tbl IN ARRAY owned_tables LOOP
    ...
    ELSIF pk_cols IS DISTINCT FROM ARRAY['id']::text[] THEN
      incompatible_pks := array_append(
        incompatible_pks,
        format('%s(expected=id,actual=%s)', tbl, array_to_string(pk_cols,','))
      );
    END IF;
  END LOOP;

  -- Duplicate ticket_id groups (would block UNIQUE(ticket_id)). Never delete/merge.
  IF to_regclass('public.support_ai_analysis') IS NOT NULL THEN
    SELECT c.relkind INTO actual_relkind
    FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
    WHERE n.nspname='public' AND c.relname='support_ai_analysis';
    IF FOUND AND actual_relkind IN ('r','p') THEN
      SELECT COUNT(*) INTO row_count FROM (
        SELECT ticket_id FROM public.support_ai_analysis GROUP BY ticket_id HAVING COUNT(*) > 1
      ) d;
      IF row_count > 0 THEN
        duplicate_count := duplicate_count + row_count;
        duplicate_details := array_append(duplicate_details, format('support_ai_analysis(ticket_id)=%s', row_count));
      END IF;
    END IF;
  END IF;

  -- UNIQUE(ticket_id): exact single-column only. Wider/partial/expression/
    ...
    IF FOUND AND actual_relkind IN ('r','p') THEN
      ...
      IF wrong_uq OR bad_exact_uq OR near_miss_uq OR expression_uq THEN
        incompatible_uniques := array_append(
          ...
          format('support_ai_analysis(expected=ticket_id,same_name_wrong=%s,bad_exact=%s,near_miss=%s,expression=%s)',
      ...
      ELSIF NOT has_uq THEN
        missing_uniques := array_append(missing_uniques, 'support_ai_analysis(ticket_id)');
      END IF;
    END IF;
  END IF;
  ...
  ELSE
    action := 'ALREADY_CORRECT'; reason_code := 'SUPPORT_AI_SCHEMA_READY'; lock_risk := 'LOW';
  END IF;
  ...
  END LOOP;
  RAISE NOTICE '045_preflight: estimated_rows %',rows_notice;
  RAISE NOTICE '045_preflight: lock_risk=%',lock_risk;
  RAISE NOTICE '045_preflight: null_required_count=% details=%',null_required_count,
    coalesce(nullif(array_to_string(null_required_details,','),''),empty_text);
  RAISE NOTICE '045_preflight: duplicate_unique_keys=%',
    coalesce(nullif(array_to_string(duplicate_details,','),''),empty_text);
  RAISE NOTICE '045_preflight: incompatible_objects=% incompatible_types=%',
  ...
  RAISE NOTICE '045_preflight: incompatible_pks=% incompatible_uniques=%',
  ...
  RAISE NOTICE '045_preflight: missing_tables=%',
    coalesce(nullif(array_to_string(missing_tables,','),''),empty_text);
  RAISE NOTICE '045_preflight: missing_columns=%',coalesce(nullif(array_to_string(missing_columns,','),''),empty_text);
  RAISE NOTICE '045_preflight: missing_defaults=% missing_not_null=%',
    coalesce(nullif(array_to_string(missing_defaults,','),''),empty_text),
    coalesce(nullif(array_to_string(missing_not_null,','),''),empty_text);
  RAISE NOTICE '045_preflight: missing_pks=% missing_uniques=%',
    coalesce(nullif(array_to_string(missing_pks,','),''),empty_text),
    coalesce(nullif(array_to_string(missing_uniques,','),''),empty_text);
  RAISE NOTICE '045_preflight: chosen_action=% reason_code=%',action,reason_code;
  RAISE NOTICE '045_preflight: KB note — duplicate seed rows are preserved; no invented business UNIQUE';

  IF action='BLOCK_AND_MANUAL_REVIEW' THEN
    RAISE NOTICE '045_preflight: BLOCK — do not apply Migration 045 until every blocker is resolved';
    RAISE EXCEPTION '045_preflight: chosen_action=% reason_code=%',action,reason_code;
  ELSIF action='SAFE_AUTO_REPAIR' THEN
    RAISE NOTICE '045_preflight: SAFE_AUTO_REPAIR — Migration 045 may repair the reported non-blocking gaps';
  ELSE
    RAISE NOTICE '045_preflight: ALREADY_CORRECT — FULL READY (reason_code=SUPPORT_AI_SCHEMA_READY)';
  END IF;
END
$preflight$;
